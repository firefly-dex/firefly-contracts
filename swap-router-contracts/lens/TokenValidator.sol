...
contract TokenValidator is ITokenValidator, IFireflyV2Callee, ImmutableState {
    string internal constant FOT_REVERT_STRING = 'FOT';
    // https://github.com/Firefly/v2-core/blob/1136544ac842ff48ae0b1b939701436598d74075/contracts/FireflyV2Pair.sol#L46
    ...
    function _validate(
        ...
        address pairAddress = FireflyV2Library.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(IFireflyV2Pair.token0.selector));
        ...
        IFireflyV2Pair pair = IFireflyV2Pair(pairAddress);
    ...
    function fireflyV2Call(
        address,
        uint256 amount0,
        uint256,
        bytes calldata data
    ) external view override {
        IFireflyV2Pair pair = IFireflyV2Pair(msg.sender);
